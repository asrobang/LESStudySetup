...
function plot_spectraTuvw(filesave, fileparam, T, u, v, w)
    # Coordinate arrays
    xu, yu, zu = nodes(u)       # u at cell faces in x
    xv, yv, zv = nodes(v)       # v at cell faces in y
    xw, yw, zw = nodes(w)       # w at cell faces in z
    ...
    axis_kwargs1 = (xlabel = "Wavenumber (rad⋅m⁻¹)",                # wavenumber k 
                    ylabel = L"E_i(k)/E_{T,v}(k_{min},z=$(depth)~m)",     # normalized wrt E at k_min
...
# Figure 1: spectra of E_T, E_u, E_v, E_w
plot_spectraTuvw(filesave, fileparam, T, u, v, w)
...
# ax = Axis(fig[1, 1]; title="z=$(depth) m", axis_kwargs1...)
...
# # klev, depth = 65, -8.4375       # corresponds to depth = -8.4375 m
# klev, depth = 70, -2.8125       # corresponds to depth = -2.8125 m
